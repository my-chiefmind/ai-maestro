...
export async function getSpec(id: string): Promise<{ content: string; version: string }> {
  ...
  return { content: body.content, version: body.version };
}

export async function putSpec(id: string, content: string, version: string): Promise<string> {
  ...
  if (!r.ok) {
    const e = await r.json().catch(() => ({}));
    if (r.status === 409) throw new SpecConflict(e.error || 'Spec changed on disk. Reload it before saving.');
    throw new Error(e.error || `spec save failed (${r.status})`);
  }
  return (await r.json()).version;
}

export class SpecConflict extends Error {}
