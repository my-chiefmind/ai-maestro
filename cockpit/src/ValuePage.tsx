...
const DIMENSIONS = ['model', 'agent', 'runtime', 'provider', 'provenance', 'stage', 'date'] as const;
...
const DIMENSION_HINT: Record<string, string> = {
  ...
  runtime: 'The harness that ran the work, such as Claude Code or Codex.',
  provider: 'The inference provider reported by telemetry or session metadata. Legacy rows remain unknown.',
  provenance: 'Orchestration work and application API calls stay separate while provider totals reconcile.',
...
function TimingStripe({ timing }: { timing: UsageTicket['timing'] }) {
  ...
  return (
    <Tooltip title={timing === 'exact' ? 'Measured from run telemetry'
      : timing === 'application' ? 'Exact application API usage; no working time claimed'
      : timing === 'mixed' ? 'Multiple usage provenances'
        : 'Inferred from session transcripts'}>
      <Box component="span" sx={{
        display: 'inline-block', width: 3, height: 18, mr: 1, verticalAlign: 'middle', borderRadius: 0.5,
        color: timing === 'estimated' ? 'text.disabled' : timing === 'application' ? 'warning.main' : 'primary.main',
        background: timing === 'exact' || timing === 'application' ? 'currentColor'
...
const CONFIDENCE_COLOR: Record<string, 'success' | 'primary' | 'warning' | 'default'> = {
  exact: 'success', high: 'primary', medium: 'warning', application: 'warning', unassigned: 'default',
};
    ...
    <Table size="small">
      <TableBody>
        {rows.slice(0, dense ? 6 : 12).map((r) => (
          <TableRow key={r.key}>
            <TableCell sx={{ border: 0, py: 0.35, fontSize: 13 }}>{shortModel(r.label || r.key)}</TableCell>
...
export default function ValuePage() {
  ...
  const dims: Dimension[] = (isPortfolio ? ['project', ...DIMENSIONS] : ['project', ...DIMENSIONS])
    .filter((d) => report.breakdown[d]) as Dimension[];
    ...
    <Container maxWidth="xl" sx={{ py: 3, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
      <Box>
        ...
        <Typography sx={{ mt: 0.8, fontSize: 11.5, color: 'text.disabled', fontFamily: 'monospace' }}>
          ...
          {c.applicationCalls !== undefined && <>{' · '}{c.applicationCalls} application calls</>}
      ...
      <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', gap: 1 }}>
        ...
        {(c.applicationCalls || 0) > 0 && <Meter label="Application calls" value={String(c.applicationCalls)} note="exact provider counters; no time inferred" />}
      </Stack>
      ...
      <Card sx={{ overflowX: 'auto' }}>
        <Table size="small" sx={{ minWidth: 900 }}>
          ...
          <TableBody>
            ...
            {report.tickets.map((tk) => (
              <Fragment key={tk.id}>
                <TableRow hover sx={{ cursor: 'pointer' }} onClick={() => setOpen(open === tk.id ? null : tk.id)}>
                  ...
                  <TableCell align="right" sx={{ fontVariantNumeric: 'tabular-nums', color: 'text.secondary' }}>
                    {tk.metrics.turns || '—'}{tk.metrics.runs ? ` +${tk.metrics.runs}r` : ''}{tk.metrics.applicationCalls ? ` +${tk.metrics.applicationCalls}a` : ''}
                  </TableCell>
            ...
            {(report.projectOnly?.applicationCalls || 0) > 0 && (
              <TableRow sx={{ '& td': { borderTop: '2px solid', borderColor: 'divider' } }}>
                <TableCell sx={{ fontFamily: 'monospace', fontWeight: 600, whiteSpace: 'nowrap', color: 'warning.main' }}>Project only</TableCell>
                {isPortfolio && <TableCell />}
                <TableCell sx={{ fontSize: 12.5, color: 'text.secondary' }}>Application usage recorded without a ticket</TableCell>
                <TableCell><Chip size="small" variant="outlined" color="warning" label="application" sx={{ fontSize: 10, height: 20 }} /></TableCell>
                {TOKEN_COLUMNS.map((col) => <TableCell key={col.key} align="right">{fmtTokens(report.projectOnly!.tokens[col.key])}</TableCell>)}
                <TableCell align="right" sx={{ fontWeight: 700 }}>{fmtTokens(report.projectOnly!.tokens.total)}</TableCell>
                <TableCell><TokenBar tokens={report.projectOnly!.tokens} /></TableCell>
                <TableCell align="right">—</TableCell><TableCell />
                <TableCell align="right">{report.projectOnly!.applicationCalls}a</TableCell><TableCell />
              </TableRow>
            )}
          </TableBody>
        </Table>
