...
import { getSpec, putSpec, SpecConflict } from './api';
import { isCurrentSpecResponse, saveStateForEdits } from './specResponseGuard.mjs';
import {
...
function TicketDrawer({ board, config, ticket: t, onClose, onPatch, onDelete }: DrawerProps) {
  ...
  const [specVersion, setSpecVersion] = useState('sha256:absent');
  const [specLoadedFor, setSpecLoadedFor] = useState<string | null>(null);
  const [specError, setSpecError] = useState('');
  const [specSaved, setSpecSaved] = useState<'idle' | 'saving' | 'saved'>('idle');
  const specRequestGeneration = useRef(0);
  const specEditGeneration = useRef(0);
  const activeSpecTicket = useRef<string | null>(null);
  activeSpecTicket.current = t?.id ?? null;
  ...
  useEffect(() => {
    const generation = ++specRequestGeneration.current;
    const ticketId = t?.id ?? null;
    const editsAtLoad = specEditGeneration.current;
    setConfirmDel(false); setSpecSaved('idle'); setSpecError(''); setSpecLoadedFor(null);
    setSpec(''); setSpecVersion('sha256:absent');
    if (ticketId) getSpec(ticketId).then((value) => {
      if (!isCurrentSpecResponse(ticketId, activeSpecTicket.current, generation, specRequestGeneration.current)) return;
      setSpecVersion(value.version);
      setSpecLoadedFor(ticketId);
      if (specEditGeneration.current === editsAtLoad) setSpec(value.content);
    }).catch((error) => {
      if (isCurrentSpecResponse(ticketId, activeSpecTicket.current, generation, specRequestGeneration.current)) setSpecError(error.message);
    });
    return () => { specRequestGeneration.current++; };
  }, [t?.id]); // eslint-disable-line react-hooks/exhaustive-deps

  const saveSpec = () => {
    if (!t || specLoadedFor !== t.id) return;
    const ticketId = t.id;
    const generation = specRequestGeneration.current;
    const editsAtSave = specEditGeneration.current;
    const contentAtSave = spec;
    setSpecSaved('saving');
    setSpecError('');
    putSpec(ticketId, contentAtSave, specVersion).then((version) => {
      if (!isCurrentSpecResponse(ticketId, activeSpecTicket.current, generation, specRequestGeneration.current)) return;
      setSpecVersion(version);
      setSpecSaved(saveStateForEdits(editsAtSave, specEditGeneration.current));
    }).catch((error) => {
      if (!isCurrentSpecResponse(ticketId, activeSpecTicket.current, generation, specRequestGeneration.current)) return;
      setSpecSaved('idle');
      setSpecError(error instanceof SpecConflict ? 'Spec changed on disk. Reload this ticket before saving.' : error.message);
    });
  };
        ...
        <>
          ...
          {edit ? (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              ...
              <SpecEditor spec={spec} setSpec={(value) => { specEditGeneration.current++; setSpec(value); }} onSave={saveSpec}
                state={specSaved} setState={setSpecSaved} error={specError} ready={specLoadedFor === t.id} />
...
function SpecEditor({ spec, setSpec, onSave, state, setState, error, ready }: {
  ...
  return (
    <Box>
      <TextField label="Spec (long-form detail — saved to board/specs/&lt;id&gt;.md)" value={spec}
        onChange={(e) => { setSpec(e.target.value); setState('idle'); }} disabled={!ready} fullWidth multiline minRows={4}
        helperText="Optional. Use for acceptance criteria, design notes, anything longer than the description." />
      {error && <Typography color="error" variant="body2" sx={{ mt: 1 }}>{error}</Typography>}
      <Box sx={{ mt: 0.6 }}>
        <Button size="small" variant="outlined" onClick={onSave} disabled={!ready || state === 'saving'}>
