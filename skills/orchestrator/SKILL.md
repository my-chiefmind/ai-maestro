...
Dispatch the **`orchestrator` agent** for **one ticket**. If the ticket needs an explicit
delivery-control checkpoint, put `tpm` first in its `agent_plan` (for example,
`["tpm", "pe", "backend"]`); the orchestrator stops if TPM returns a blocker. Pass along
anything the user asked for — a specific ticket id (resume), or "what do you see" (discovery:
report the board's state and dispatch nothing).
