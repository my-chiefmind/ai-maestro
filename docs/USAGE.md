...
Codex is capability-checked and, when supported, run once with `codex exec --json`. Maestro reads
the thread id, final agent message, terminal status and exact provider counters from that stream.
Cached input is subtracted from Codex's inclusive `input_tokens` into disjoint Maestro bins; a
malformed or unavailable block remains unavailable rather than being reported as zero. Runtime
(`codex`) and inference provider (`openai`, or another configured provider) are separate fields.
...
Codex rollouts are read from both `$CODEX_HOME/sessions` and
`$CODEX_HOME/archived_sessions`. Per-response ids are deduplicated; older cumulative snapshots
are converted to positive deltas and reset safely after a regression. Exact run telemetry wins
over reconstruction for the same runtime-qualified session. DeepSeek ingestion is intentionally
deferred; the provider dimension is already ready to aggregate it without mislabelling runtime.
...
Cache keys and cwd ownership markers are SHA-256 fingerprints. Raw transcript and project paths,
prompts, responses, tool I/O and credentials are never persisted in the cache or returned by a
report.
...
```bash
maestro usage                     # breakdowns include model/agent/runtime/provider/stage/date
maestro usage --csv tickets       # or model | agent | runtime | provider | stage | date
maestro usage --json
...
Consumers use the supported package facade rather than deep-importing scripts:

```js
import { buildUsageReport, buildPortfolioUsage, usageToCsv } from "@mychiefmind/ai-maestro/usage";
const report = buildUsageReport({ boardPath: "./board" });
```

### Filtering several dimensions safely

The public report includes `usageSlices`, a schema-versioned daily table for companion UIs:

```js
import { USAGE_SLICE_SCHEMA, USAGE_SLICE_DIMENSIONS } from "@mychiefmind/ai-maestro/usage";

report.usageSlices.schema === USAGE_SLICE_SCHEMA; // 1
report.usageSlices.dimensions;                    // the frozen eight-dimension contract
report.usageSlices.rows;                          // joint observations, not inferred combinations
```

Every row keeps the complete `projectKey + ticketId + scope + provider + model + runtime +
provenance + UTC date` tuple and a `metrics` block. This matters when one project has DeepSeek
application calls and OpenAI Codex runs: combining the older one-dimensional breakdowns would
invent combinations that never happened. The slice rows are instead built in the same pass over
the canonical observations as the totals, and all additive counters reconcile exactly.

The three scopes are `ticket`, `project-only` (an intentional application call without a ticket),
and `unassigned` (historical agent work whose ticket could not be proved). Missing provider,
model, runtime, or provenance is `unknown`. Provider counters that were unavailable are retained
with zero token counters and a positive `unavailableUsageRuns`; they are not silently discarded
or called free. `project` is a display label, while the opaque `projectKey` prevents same-named
projects and duplicate ticket ids from collapsing in a portfolio. The rows never contain raw
paths, run/event/session ids, evidence strings, prompts, responses, or transcript content.

### Recording DeepSeek application usage

Application calls are a separate append-only ledger. Maestro accepts only aggregate counters —
never API keys, prompts, responses, headers, or raw logs. Keep the key in the environment, make
the request in your application, and pass only `body.usage` through the mapper:

```js
import { appendApplicationUsage, mapDeepSeekUsage } from "@mychiefmind/ai-maestro/usage";
...
appendApplicationUsage({
  ...
  event: {
    project: "my-project",
    ticketId: "T-123", // optional; omit or use null for project-only usage
    provider: "deepseek",
    model: body.model || "deepseek-chat",
    timestamp: new Date().toISOString(),
    operation: "chat-completion",
    usage: mapDeepSeekUsage(body.usage), // only the usage object crosses this boundary
    idempotencyKey: response.headers.get("x-request-id") || applicationRequestId,
  },
});
```

The idempotency key is mandatory but is stored only as a SHA-256 digest. Replaying an identical
event is a no-op; reusing its key for different counters is a typed conflict. The project must
match `config.json`, and an optional ticket must exist on the live board or in its archive.
Malformed ledger lines are skipped and counted by reports, but block subsequent writes because
exactly-once behavior cannot be proven while the ledger is ambiguous.
...
- **Older Codex versions without `exec --json`** are run once in legacy mode and recorded with an
  exact duration and `usage: null` / `usageSource: "none"`. A zero would read as "this stage was
