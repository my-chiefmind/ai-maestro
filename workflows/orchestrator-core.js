...
const BOARD_SCHEMA = {
  type: "object",
  required: ["epics", "tickets", "eligibility"],
  properties: {
    epics: { type: "array" },
    tickets: { type: "array" },
    archiveTickets: { type: "array", items: { type: "object" } },
    // Canonical, locked dispatch verdicts from `maestro ticket eligibility --json`.
    eligibility: {
      type: "object",
      required: ["ok", "verdicts", "version", "archiveVersion", "planVersion"],
      properties: {
        ok:             { type: "boolean" },
        version:        { type: "string" },
...
const ELIGIBILITY_CODES = new Set([
  "status", "human-gate", "dependency-missing", "dependency-unsatisfied",
  "malformed-reference", "plan-scope", "initiative-unassigned",
  "initiative-unknown", "initiative-cross",
]);

function eligibilityFor(board, ticketId) {
  const rows = board && board.eligibility && board.eligibility.verdicts;
  return Array.isArray(rows) ? rows.find(row => row && row.ticketId === ticketId) || null : null;
}

function canonicalEligibilityErrors(board) {
  const snapshot = board && board.eligibility;
  if (!snapshot || snapshot.ok !== true) return ["canonical eligibility snapshot is missing or failed"];
  for (const key of ["version", "archiveVersion", "planVersion"]) {
    if (typeof snapshot[key] !== "string" || !snapshot[key].startsWith("sha256:")) {
      return [`canonical eligibility snapshot has invalid ${key}`];
    }
  }
  if (!Array.isArray(snapshot.verdicts)) return ["canonical eligibility verdicts are missing"];
  const tickets = (board && board.tickets) || [];
  if (snapshot.verdicts.length !== tickets.length) return ["canonical eligibility snapshot does not match the active board"];
  const expected = tickets.map(ticket => ticket.id);
  const actual = snapshot.verdicts.map(verdict => verdict && verdict.ticketId);
  if (expected.some((id, index) => id !== actual[index])) return ["canonical eligibility snapshot is not in active-board order"];
  const errors = [];
  for (const verdict of snapshot.verdicts) {
    if (!verdict || !["eligible", "blocked"].includes(verdict.state) || verdict.eligible !== (verdict.state === "eligible") || !Array.isArray(verdict.reasons)) {
      errors.push(`invalid canonical eligibility verdict for ${verdict && verdict.ticketId || "unknown ticket"}`);
      continue;
    }
    if (verdict.reasons.some(reason => !reason || !ELIGIBILITY_CODES.has(reason.code))) {
      errors.push(`unknown canonical eligibility reason for ${verdict.ticketId}`);
    }
    if (verdict.eligible && verdict.reasons.length) errors.push(`eligible verdict for ${verdict.ticketId} contains blockers`);
    if (!verdict.eligible && !verdict.reasons.length) errors.push(`blocked verdict for ${verdict.ticketId} has no reason`);
  }
  return errors;
}

function canonicalReadyTickets(board) {
  return (board.tickets || []).filter(ticket => eligibilityFor(board, ticket.id)?.eligible === true);
}

function canonicalBlockedTickets(board) {
  return (board.tickets || []).filter(ticket =>
    ticket.status === "todo" && eligibilityFor(board, ticket.id)?.state === "blocked"
  );
}

function canonicalPickNextTicket(board) {
  const todos = canonicalReadyTickets(board);
  if (todos.length === 0) return null;
  const priorities = ["P0", "P1", "P2", "P3"];
  for (const priority of priorities) {
    const match = todos.find(ticket => ticket.priority === priority);
...
function preflightBoard(board) {
  // Dispatch authorization comes only from the locked public API snapshot. Missing or
  // malformed verdict data blocks the run instead of falling back to copied rules.
  const errors = canonicalEligibilityErrors(board);
  const tickets = (board && board.tickets) || [];
  ...
  const visit = (id, stack) => {
    ...
    state.set(id, 0);
    const t = byId.get(id);
    if (t) for (const dep of (Array.isArray(t.depends_on) ? t.depends_on : [])) if (byId.has(dep)) visit(dep, [...stack, id]);
    state.set(id, 1);
  };
...
async function readBoard() {
  ...
  Do not read plan.json yourself and do not decide scope, ownership, dependencies, status or
  human gates from board fields; all dispatch permission comes from \`eligibility\`.`,
...
async function claimBoard(id, snapshot, coord) {
  ...
  record or worktree; return the verdict and conflicts exactly as emitted.`,
    { label: `claim-${id}` }
  );
}
...
async function confirmTicket(ticket, repoPath, agentType, branch) {
  ...
  Maestro's canonical eligibility API has already authorized this ticket. Do NOT recalculate
  status, dependencies, human gates, plan scope, or initiative ownership. Only flag an
  environment blocker that makes the named repo/worktree impossible to use. Do NOT flag test
  command wording, agent assignments, or implementation details. Return ticketId, reason (why
  this ticket is a good next pick), repoPath, area, and any blockers found.`,
...
async function eligibilityBlockedReport(board, blocked) {
  const lines = blocked.map(ticket => {
    const verdict = eligibilityFor(board, ticket.id);
    return `${ticket.id} (${ticket.name}): ${verdict.reasons.map(reason => `${reason.code}: ${reason.message}`).join("; ")}`;
  }).join(" | ");
  const outcome = await agent(
    `No todo ticket may be dispatched according to Maestro's canonical eligibility API. The
  blocked tickets and stable reasons are: ${lines}. Return outcome="idle" with a concise
  summary that preserves the ticket ids and reason codes. Do NOT clear a gate, invent a
  dependency result, edit board files, or start a ticket.`,
    { label: "eligibility-blocked-report", schema: OUTCOME_SCHEMA }
  );
  log(`Eligibility blocked: ${outcome.summary}`);
  return outcome;
}
...
${BOARD_EPILOGUE}
  ...
  const claim = await claimBoard(ticket.id, board.eligibility, {
    executionMode: "multi-agent", agentPlan: plan,
    currentAgent: plan[0] || "", nextAgent: plan[1] || "",
  });
  if (!claim || claim.claimed !== true) {
    const reasons = claim?.verdict?.reasons ?? [];
    const summary = reasons.length
      ? reasons.map(reason => `${reason.code}: ${reason.message}`).join("; ")
      : `eligibility inputs changed (${(claim?.conflicts ?? []).map(row => row.source).join(", ") || "unknown"})`;
    return { outcome: "blocked", ticketId: ticket.id, summary: `claim refused — ${summary}` };
  }
...
async function doStartNext() {
  ...
  const ticket = canonicalPickNextTicket(board);
  if (!ticket) {
    const blocked = canonicalBlockedTickets(board);
    return blocked.length ? await eligibilityBlockedReport(board, blocked) : await idleReport(board);
  }
...
async function doStart(id) {
  ...
  const verdict = eligibilityFor(board, id);
  if (!verdict || !verdict.eligible) {
    const reasons = verdict ? verdict.reasons.map(reason => `${reason.code}: ${reason.message}`).join("; ") : "canonical verdict missing";
    return { outcome: "blocked", ticketId: id, summary: `ticket ${id} is not eligible — ${reasons}` };
  }
  return await startTicket(ticket, board);
}
