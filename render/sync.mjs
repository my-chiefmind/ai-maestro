...
if (config.targets?.workflow && claudeEnabled) {
  ...
  // QA, principal delivery, and delivery TPM independently assess evidence; every other
  // rostered agent writes. The orchestrator itself is the harness, not a stage.
  const READERS = new Set(["qa", "pd", "tpm"]);
  const agentsMap = {};
  for (const f of agentFiles) {
