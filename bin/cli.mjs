 ...
 *   maestro spec [...]        (thin passthrough to scripts/spec-write.mjs)
 *
...
const BOARD_USER_FILES = new Set(["data.json", "archive.json", "application-usage.jsonl"]);
...
function writeVendorPackageJson(dest) {
  const pkg = {
    ...
    scripts: {
      ...
      spec: "node scripts/spec-write.mjs",
...
function help() {
  ...
  spec        List, read, and safely write versioned board specs
              Reads emit exact content. Writes require --expect-version and take the shared
              board lock, so a stale editor cannot overwrite a newer change. Use --file - for
              stdin and --json for stable machine output. Run 'maestro spec --help' for ops.
...
const COMMANDS = [
  ...
  { key: "spec", label: "List, read, and safely write versioned specs" },
