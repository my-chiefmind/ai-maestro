...
import {
  buildUsageReport, usageToCsv, DIMENSIONS, buildPortfolioUsage,
  discoverProjects, projectsFromRegistry,
} from "./usage-api.mjs";
...
if (has("help") || argv[0] === "-h") {
  ...
  maestro usage            time and tokens per ticket, by project, model, agent, runtime,
                             provider, provenance, stage and date
  ...
  Flags:
    ...
    --csv [view]           print CSV: tickets (default) | project | ${DIMENSIONS.join(" | ")} | provenance
  ...
  Reading Claude Code and Codex transcripts is opt-in — set "usage": { "scanTranscripts": true } in
...
if (has("all") || flag("discover") || flag("registry")) {
  ...
  report = buildUsageReport({ boardPath: boardDir, useCache });
}
...
process.stdout.write(`transcripts: ${report.enabled.transcripts ? `on (${c.transcriptSessions} sessions)` : "off (opt-in)"}   telemetry: ${c.exactRuns} measured runs   application: ${c.applicationCalls || 0} calls\n\n`);
...
if (report.projectOnly?.applicationCalls > 0) {
  process.stdout.write(`  ${pad("—", 7)}${isPortfolio ? pad("", 14) : ""}${pad("application", 11)}${tokCells(report.projectOnly.tokens)}${rpad("—", 8)}${rpad("", 9)}  (project-only application usage; ${report.projectOnly.applicationCalls} calls)\n`);
}

for (const d of Object.keys(report.breakdown)) {
  const rows = /** @type {any[]} */ (report.breakdown[d] || []).slice(0, d === "date" ? 7 : 8);
  if (!rows.length) continue;
  process.stdout.write(`\n  BY ${d.toUpperCase()}\n`);
  for (const r of rows) {
    process.stdout.write(`  ${pad(r.key, 30)}${rpad(M(r.tokens.total), 8)}${rpad(H(r.estimatedActiveMs + r.exactMs), 8)}  ${r.turns} turns${r.runs ? `, ${r.runs} runs` : ""}${r.applicationCalls ? `, ${r.applicationCalls} application calls` : ""}\n`);
  }
}
