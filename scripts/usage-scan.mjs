...
// @ts-check
/**
 * usage-scan.mjs — read local Claude Code and Codex sessions into a privacy-safe event
...
import { join, dirname, basename, resolve, parse } from "path";
import { createHash } from "crypto";
import { codexRolloutFiles, distillCodexRollout } from "./usage-codex.mjs";
 ...
 * one project uses `kit-096` and another used `tl-226`, and a hardcoded `T-` reported 0% of
...
/**
 * @typedef {{
 *   ts: number, kind: "turn" | "evidence", cwd: string, cwdAncestors?: string[], branch: string | null, sessionId: string,
 ...
 *
 * This is the whole of nested-repo correctness. `~/source/monorepo` and
 * `~/source/monorepo/apps/web` are both real projects with their own boards,
...
export function eventsForRoots(events, roots, excludeRoots = []) {
  const owns = (/** @type {UsageEvent} */ e, /** @type {string} */ root) => e.cwd
    ? ownsCwd(e.cwd, [root], excludeRoots)
    : Array.isArray(e.cwdAncestors) && e.cwdAncestors.includes(pathFingerprint(root));
  return events.filter((e) => {
    const mine = roots.filter((r) => owns(e, r));
    if (!mine.length) return false;
    const deepest = Math.max(...mine.map((r) => r.length));
    return !excludeRoots.some((r) => owns(e, r) && r.length > deepest);
  });
}

const pathFingerprint = (/** @type {string} */ p) => createHash("sha256").update(resolve(p)).digest("hex");
const cwdAncestors = (/** @type {string} */ cwd) => {
  if (!cwd) return [];
  const out = []; let p = resolve(cwd); const root = parse(p).root;
  while (true) { out.push(pathFingerprint(p)); if (p === root) break; p = dirname(p); }
  return out;
};
...
function loadCache(cacheFile) {
  ...
  try {
    ...
    if (c?.schema !== CACHE_SCHEMA) return { schema: CACHE_SCHEMA, files: {}, reset: true };
    return { schema: CACHE_SCHEMA, files: c.files || {} };
  } catch { return { schema: CACHE_SCHEMA, files: {}, reset: true }; }
}
 ...
 * @param {{ roots: string[], excludeRoots?: string[], projectsDir?: string, codexHome?: string, cacheFile?: string, useCache?: boolean }} opts
...
export function scanTranscripts(opts) {
  ...
  /** @param {string} filePath @param {string | null} agentType @param {(p:string,o?:any)=>any[]} [parser] */
  const take = (filePath, agentType, parser = distill) => {
    let stat;
    try { stat = statSync(filePath); } catch { return; }
    const sig = `${stat.mtimeMs}:${stat.size}`;
    const cacheKey = pathFingerprint(filePath);
    const cached = cache.files[cacheKey];
    let distilled;
    if (cached && cached.sig === sig) {
      distilled = cached.events;
    } else {
      distilled = parser(filePath, { agentType }).map((e) => ({
        ...e, cwd: "", cwdAncestors: cwdAncestors(e.cwd),
        runtime: e.runtime || "claude", provider: e.provider || "anthropic",
      }));
      cache.files[cacheKey] = { sig, events: distilled };
      dirty = true;
    }
    files++;
  ...
  // A caller supplying a custom Claude fixture directory should not accidentally scan the
  // developer's real Codex home. Tests/importers opt into a custom Codex tree independently.
  if (opts.codexHome !== undefined || opts.projectsDir === undefined) {
    for (const filePath of codexRolloutFiles(opts.codexHome, roots)) take(filePath, null, distillCodexRollout);
  }
  ...
  const deduped = events.filter((e) => {
    ...
    const key = `${e.runtime || "unknown"}:${e.sessionId}:${e.responseId}`;
    if (seenResponses.has(key)) return false;
    seenResponses.add(key); return true;
  });
  return { events: deduped, sessions: sessions.size, files, agentTypes };
}
