...
import {
  buildUsageReport, rootsForBoard, DIMENSIONS, opaqueProjectKey,
...
export const PORTFOLIO_DIMENSIONS = [...new Set(["project", ...DIMENSIONS, "provenance"])];
...
function mergeMetrics(a, b) {
  ...
  a.turns += b.turns || 0;
  a.runs += b.runs || 0;
  if (a.applicationCalls !== undefined || b.applicationCalls !== undefined) a.applicationCalls = (a.applicationCalls || 0) + (b.applicationCalls || 0);
  a.usageRuns += b.usageRuns || 0;
  a.unavailableUsageRuns += b.unavailableUsageRuns || 0;
...
function mergeBuckets(into, rows) {
  for (const r of rows || []) {
    ...
    if (!b) into.set(r.key, (b = { key: r.key, ...(r.label ? { label: r.label } : {}), ...zero() }));
    mergeMetrics(b, r);
  }
}
 ...
 *   projectsDir?: string, codexHome?: string, cacheFile?: string, env?: NodeJS.ProcessEnv,
 *   useCache?: boolean, config?: any,
 * }} opts
 */
export function buildPortfolioUsage(opts) {
  const entries = opts.projects.filter((p) => p.kitDir);
  const notSetUp = opts.projects.filter((p) => !p.kitDir).map((p) => ({ key: opaqueProjectKey(p.path), name: p.name, ok: false, error: "not set up (no maestro kit dir)" }));
  ...
  for (const p of entries) {
    ...
    try {
      report = buildUsageReport({
        ...
        projectsDir: opts.projectsDir, codexHome: opts.codexHome, cacheFile: opts.cacheFile,
        env: opts.env, useCache: opts.useCache,
        ...(opts.config !== undefined ? { config: opts.config } : {}),
      });
    } catch (e) {
      coverage.projectsFailed++;
      projectRows.push({ key: opaqueProjectKey(p.path), name: p.name, ok: false, error: "project usage could not be read" });
      continue;
    }
    ...
    if (report.projectOnly) mergeMetrics(projectOnly, report.projectOnly);
    for (const d of DIMENSIONS) mergeBuckets(/** @type {Map<string, any>} */(breakdown.get(d)), /** @type {any[]} */ (report.breakdown[d] || []));
    if (report.breakdown.provenance) mergeBuckets(/** @type {Map<string, any>} */(breakdown.get("provenance")), report.breakdown.provenance);
    // The project dimension exists only up here — a single board cannot produce it.
    const key = opaqueProjectKey(p.path);
    mergeBuckets(/** @type {Map<string, any>} */(breakdown.get("project")), [{ key, label: p.name, ...report.totals }]);

    const c = report.coverage;
    coverage.turns += c.turns; coverage.attributed += c.attributed;
    coverage.exactRuns += c.exactRuns;
    if (c.applicationCalls !== undefined || c.applicationSkippedLines !== undefined) {
      hasApplication = true;
      coverage.applicationCalls = (coverage.applicationCalls || 0) + (c.applicationCalls || 0);
      coverage.applicationSkippedLines = (coverage.applicationSkippedLines || 0) + (c.applicationSkippedLines || 0);
    }
    ...
    for (const t of report.tickets) tickets.push({ ...t, project: p.name, projectKey: key });

    // Project identity is part of the key, so identical ticket ids in different projects
    // remain separate. Merge only truly identical joint-dimension rows.
    for (const source of report.usageSlices?.rows || []) {
      const row = { ...source, project: p.name, projectKey: key };
      const sliceKey = JSON.stringify(USAGE_SLICE_DIMENSIONS.map((d) => row[d]));
      let target = usageSlices.get(sliceKey);
      if (!target) {
        target = { ...row, metrics: zeroSliceMetrics() };
        usageSlices.set(sliceKey, target);
      }
      mergeMetrics(target.metrics, source.metrics);
    }

    projectRows.push({
      key, name: p.name, ok: true,
  ...
  return {
    ...
    breakdown: Object.fromEntries((hasApplication ? PORTFOLIO_DIMENSIONS : PORTFOLIO_DIMENSIONS.filter((d) => d !== "provenance"))
      .map((d) => [d, listOf(/** @type {Map<string, any>} */(breakdown.get(d)))])),
  };
}
