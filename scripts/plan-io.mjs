...
export class PlanConflictError extends BoardConflictError {
  constructor(message, detail = {}) { super(message, detail); this.name = "PlanConflictError"; this.code = "EPLANCONFLICT"; }
}

export class PlanValidationError extends Error {
  constructor(message, detail = {}) { super(message); this.name = "PlanValidationError"; this.code = "EPLANVALIDATION"; Object.assign(this, detail); }
}
...
export function readPlan(planPath) {
  ...
  try {
    ...
    throw new PlanInputError(`Invalid JSON in ${planPath}: ${e.message}. Fix the file before writing to it.`, { path: planPath });
  }
}
 ...
 */
export function mutatePlan({ planPath, mutate, expectVersion, projectName = "Project", op = "plan-write", dryRun = false, lockOptions = {} }) {
  ...
  return withBoardLock(dir, () => {
    ...
    const next = normalisePlan(output?.plan ?? output ?? current);
    const result = output?.plan ? output.result : undefined;
