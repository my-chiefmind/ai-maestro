...
export function attribute(events, index, opts = {}) {
  ...
  const sessionKey = (/** @type {UsageEvent} */ e) => `${e.runtime || "claude"}:${e.sessionId}`;
  const exact = (/** @type {UsageEvent} */ e) => exactSessions.has(sessionKey(e)) || exactSessions.has(e.sessionId);
  ...
  /** @type {Array<{ ts: number, ticketId: string | null, confidence: string, evidence: string | null, model: string, agentType: string, sessionId: string, branch: string | null, usage: Usage, activeMs: number, runtime: string, provider: string }>} */
  const turns = [];
  let skippedExact = 0;
  for (const e of events) if (exact(e) && e.kind === "turn") skippedExact++;

  for (const [, stream] of bySession) {
