...
/** `./A\\b//c` → `a/b/c` (paths compared case-insensitively); a trailing `/` (a directory) becomes `dir/**`. */
function normGlob(glob) {
  const g = String(glob).replace(/\\/g, "/").replace(/^(\.\/)+/, "").replace(/\/{2,}/g, "/").toLowerCase();
  return g.endsWith("/") ? `${g}**` : g;
}

/** @param {string} g */
function isLiteral(g) { return !/[*?[{]/.test(g); }

/** Is one literal the other, or a directory containing it? Extensions prove nothing. */
function nested(a, b) { return a === b || a.startsWith(`${b}/`) || b.startsWith(`${a}/`); }
 ...
 *
 * Paths are compared case-insensitively. Two literals overlap iff one equals or is a directory
 * prefix of the other (extensions prove nothing: `config.d` may be a folder). A literal vs a
 * glob overlaps when the glob matches it or lives under it. Globs are
...
export function globsIntersect(a, b) {
  ...
  // Literal paths (no wildcard) are decided exactly rather than by their directory — two
  // different files in one folder never collide. A trailing `/` names a directory (`dir/**`).
  const la = isLiteral(a);
  const lb = isLiteral(b);
  if (la && lb) return nested(a, b);
  if (la || lb) {
    const [lit, glob] = la ? [a, b] : [b, a];
    // `[`/`{` are not understood by globMatches — fall through to the conservative rules.
    if (!/[[{]/.test(glob)) {
      if (globMatches(lit, glob)) return true;
      const prefix = literalPrefix(glob);
      // The glob lives under the literal (which may be a directory).
      if (prefix !== "" && prefix.startsWith(`${lit}/`)) return true;
      // Only direct children of `prefix` can match (`dir/*.css`): the literal itself was
      // rejected, and anything under it would be deeper than one level — provably disjoint.
      const rest = glob.slice(prefix.length);
      if (prefix !== "" && lit.startsWith(prefix) && !rest.includes("/") && !rest.includes("**")) return false;
      // Unrelated anchored directories: disjoint.
      if (prefix !== "" && !lit.startsWith(prefix)) return false;
      // Anything else is undecided — fall through to the conservative rules.
    }
  }
...
export function touchesSerialFile(ticket, config) {
  ...
  for (const serial of serialFiles(config)) {
    const needed = literalSegments(String(serial).toLowerCase());
...
export function touchesOf(ticket) {
  ...
  // Same normalisation as globsIntersect: `\` → `/`, no leading `./`, no `//`, lower case.
  return t.filter((x) => typeof x === "string" && x.trim()).map((x) =>
    x.trim().replace(/\\/g, "/").replace(/^(\.\/)+/, "").replace(/\/{2,}/g, "/").toLowerCase());
}
