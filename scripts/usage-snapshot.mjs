...
function dimensionPanel(rows, label) {
  ...
  <table class="dim">
    ...
    ${rows.slice(0, 10).map((r) => `<tr>
      <th scope="row">${esc(r.label || r.key)}</th>
...
export function renderUsageSnapshot(report, opts = {}) {
  ...
  const rows = report.tickets.slice(0, top).map((/** @type {any} */ tk) => {
    ...
    return `<tr class="row row--${esc(tk.timing)}">
      <td class="id"><span class="stripe stripe--${esc(tk.timing)}" title="${tk.timing === "exact" ? "Measured from run telemetry" : tk.timing === "application" ? "Exact application API usage" : tk.timing === "mixed" ? "Multiple usage provenances" : "Inferred from transcripts"}"></span>${esc(tk.id)}</td>
...
.stripe--application { background: var(--warn); }
...
.chip--application { color: var(--warn); border-color: var(--warn); }
...
<div class="wrap">
  <header>
    ...
    <div class="provenance">
      ...
      <span>Application calls <b>${c.applicationCalls || 0}</b></span>
  ...
  <section class="sec">
    ...
    <div class="tablewrap">
      <table class="ledger">
        ...
        <tbody>
${rows || `<tr><td colspan="${(isPortfolio ? 10 : 9) + TOKEN_COLUMNS.length}" class="muted" style="padding:24px">No usage recorded yet.</td></tr>`}${report.unassigned.turns > 0 ? `<tr class="unassigned">
          ...
          <td></td>
        </tr>` : ""}${report.projectOnly?.applicationCalls > 0 ? `<tr class="unassigned">
          <td class="id">Project only</td>${isPortfolio ? "<td></td>" : ""}
          <td class="name"><span class="ttl">Application usage recorded for the project without a ticket</span></td>
          <td><span class="chip chip--application">application</span></td>
          ${tokenCells(report.projectOnly.tokens)}
          <td class="num strong">${fmtTokens(report.projectOnly.tokens.total)}</td>
          <td class="barcell">${bar(report.projectOnly.tokens)}</td>
          <td class="num">—</td><td></td><td class="num muted">${report.projectOnly.applicationCalls} calls</td><td></td>
        </tr>` : ""}
        </tbody>
      </table>
    </div>
  ...
  <section class="sec">
    <div class="sechead">
      <h2>Breakdowns</h2>
      <p>The same totals cut by model, agent, runtime, provider, stage and date. Tokens first, working time second.</p>
    </div>
    <div class="grid">
      ...
      ${dimensionPanel(/** @type {any[]} */ (report.breakdown.provider), "By provider")}
      ${dimensionPanel(/** @type {any[]} */ (report.breakdown.provenance || []), "By provenance")}
