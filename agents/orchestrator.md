...
6. **Resolve, then run the plan.** Two things are computed from policy, not taken literally:
   ...
   **When `tpm` blocks**, record the specific delivery blocker and stop — do not dispatch
   implementation speculatively. **When `qa` (or `pd`) blocks**, route the fix to an
   implementation agent chosen from the
